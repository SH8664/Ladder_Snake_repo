...
void Output::DrawCardNumber(const CellPosition& cellPos, int cardNum) const
{
	...
	pWind->DrawInteger(x, y + UI.ToolBarHeight, cardNum);
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_dELRTE_GAME_OBJECT] = "images\\delete_object.jpg";
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	pWind->DrawRectangle(cellStartX, cellStartY + UI.ToolBarHeight, cellStartX + UI.CellWidth, cellStartY + UI.ToolBarHeight + UI.CellHeight);
	...
	pWind->DrawInteger(x, y + UI.ToolBarHeight, cellNum);
...
void Output::DrawPlayer(const CellPosition& cellPos, int playerNum, color playerColor) const
{
	...
	pWind->SetBrush(playerColor);
	pWind->DrawCircle(x, y + UI.ToolBarHeight, radius);
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	pWind->DrawLine(x12, y1 + UI.ToolBarHeight, x12, y2 + UI.ToolBarHeight);
	...
	pWind->DrawLine(x34, y1 + UI.ToolBarHeight, x34, y2 + UI.ToolBarHeight);
	...
	for (int i = GetCellStartY(fromCell); i > GetCellStartY(toCell); i -= UI.CellHeight)

		pWind->DrawLine(x12, i + UI.ToolBarHeight, x34, i + UI.ToolBarHeight);
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int Y[4];
	X[0] = x12;
	Y[0] = y1 + UI.ToolBarHeight;
	X[2] = x12;
	Y[2] = y1 + yChange + UI.ToolBarHeight;
	X[3] = x12 - xChange / 2;
	Y[3] = y1 + yChange / 2 + UI.ToolBarHeight;
	X[1] = x12 + xChange / 2;
	Y[1] = y1 + yChange / 2 + UI.ToolBarHeight;
